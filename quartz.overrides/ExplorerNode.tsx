// Quartz v4.4.0 quartz/components/ExplorerNode.tsx 커스텀 버전.
// 배포 시 deploy.yml 이 이 파일로 그 자리를 덮어쓴다.
//
// 원본과 동일하되, 파일/폴더 링크에 title(hover 툴팁) 속성만 추가했다.
// 이 저장소는 문서 제목이 길어서(예: "[기술] SECURITY_CODE 기반 멀티테넌시 구조
// (TickerMiddleware)") 좌측 탐색기에서 여러 줄로 줄바꿈되면 트리가 들쭉날쭉해
// 보인다. quartz.custom.scss 에서 말줄임(ellipsis) 처리를 같이 하고, 잘린
// 전체 이름은 이 title 속성으로 마우스오버 시 확인할 수 있게 했다.
// @ts-ignore
