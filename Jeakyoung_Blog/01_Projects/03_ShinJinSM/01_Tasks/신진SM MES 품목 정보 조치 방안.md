...
| **요청자** | 신진SM 현장(재고조정 담당) |
| **요청일** | 2026-09-10 |
| **대상 시스템** | MES(재고조정 엑셀 업로드/조회), MSSQL |
| **관련 화면·프로그램** | `SP_WMA624_01_LIST` (재고조정 조회용 SP), `TCO403`(품목마스터), `TMA922`(재고이력) |

## 🔍 현상 및 원인

동일 품목이 등록 사업장(`TCO403.PLANT_CODE`)과 실제 재고 보유 사업장(`TMA922.PLANT_CODE`)이 다른 경우, `SP_WMA624_01_LIST`가 두 값이 같다는 전제로 조인(`A.PLANT_CODE = B.PLANT_CODE`)하고 있어 조회 자체가 안 되는 구조적 문제. 품목이 여러 사업장 창고에 걸쳐 재고를 가질 수 있다는 걸 SP가 반영하지 못했던 것.
...
2026-09-04 적용 완료. 적용 후 회귀 여부(정상 사업장 단일 케이스가 깨지지 않는지)는 이 문서에 별도 기록 없음.

## 🔗 참고

- [[신진SM MES 수정사항 (26.09.08)]] — 같은 WMA624 재고조정 화면의 후속 개선(붙여넣기 배치화)
- [[신진SM 개발 이력]]
