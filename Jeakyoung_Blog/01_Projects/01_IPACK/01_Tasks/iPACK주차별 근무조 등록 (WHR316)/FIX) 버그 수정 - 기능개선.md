...
주차별 근무조 등록(WHR316) 개발 중 발견된 세 가지 버그: ① 다른 주차 선택 시 INSERT가 아니라 계속 UPDATE만 동작, ② 주차 조회 시 월 경계(예: 9월 1주차가 8월 31일부터 시작하는 경우) 필터링 누락, ③ 콤보박스 선택이 조회에 바로 반영되지 않음(버튼을 눌러야만 조회).
...
- [x] `WHR316_VW_btnReg`가 현재 `WHR316_CT.js`(358~392행)에 실제로 존재하며, 이 문서가 언급한 대로 `combo_05`(`WG_FROM_DATE`)를 store에 적재하는 로직이 그대로 남아있다.
- [x] `onComboChange`(35~72행)에 `combo_05` 케이스가 `me.onView()`를 호출하는 코드로 남아있어, "콤보 선택시 바로 조회" 수정 내용과 일치한다.
- [ ] 이 문서가 인용한 `SP_WHR316_01_IUD` 프로시저 본문은 DB에만 있고 레포에는 없어 원문 그대로인지는 미확인.

## 🔗 참고

- [[01_Projects/01_IPACK/02_TechDocs/근태_2기능스펙/[기술] WHR316 주차별 근무조 등록|WHR316 주차별 근무조 등록 스펙]]
- [[01_Projects/01_IPACK/01_Tasks/iPACK주차별 근무조 등록 (WHR316)/iPACK주차별 근무조 등록 (WHR316)|iPACK주차별 근무조 등록 (WHR316) 원본 작업 기록]]
