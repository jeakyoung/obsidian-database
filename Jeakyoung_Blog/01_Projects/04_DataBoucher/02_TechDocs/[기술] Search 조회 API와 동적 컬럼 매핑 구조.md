...
| 엔드포인트 | SP | 비고 |
|:--|:--|:--|
| `POST CustomerList` | `SP_APP_CUSTOMER_LIST` | `CustKind`, `CustomerName` — [[01_Projects/04_DataBoucher/01_Tasks/데이터 바우처 거래처 검색 파라미터 정리 08.07]]에서 파라미터 정리됨 |
...
- [[01_Projects/04_DataBoucher/01_Tasks/데이터 바우처 거래처 검색 파라미터 정리 08.07]]
- [[01_Projects/04_DataBoucher/02_TechDocs/[기술] 입고(Input) 등록과 임베디드 이미지 동시 저장 (InputSaveService)]] — 같은 `Dictionary` 동적 응답 패턴을 `InputListController`에서도 사용
