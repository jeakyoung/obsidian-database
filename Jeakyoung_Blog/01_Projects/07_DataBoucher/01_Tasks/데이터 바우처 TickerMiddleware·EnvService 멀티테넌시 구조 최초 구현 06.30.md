...
> 회사별(`DevEnvService`/`StgEnvService`/`PrdEnvService`)로 나뉘어 있던 환경 서비스를 `SECURITY_CODE` 헤더 기반 `TickerMiddleware` + 단일 `EnvService`로 통합 — 이 구조가 이후 [[01_Projects/07_DataBoucher/02_TechDocs/[기술] SECURITY_CODE 기반 멀티테넌시 구조 (TickerMiddleware)]]의 원형.
